...
pragma Elaborate (GNAT.Dynamic_HTables);
pragma Elaborate (GNAT.Lists);
pragma Elaborate (GNAT.Sets);
...
package body Sem_Elab is
   ...
   --  |  |                                                                 |
   --  |  +--> Check_Declaration_Scenarios                                  |
   --  |  +--> Check_Library_Body_Scenarios                                 |
   --  |  +--> Check_Library_Spec_Scenarios                                 |
   --  |  |    |                                                            |
   ...
   type Scenario_Rep_Id is range 0 .. Integer'Last;
   ...
   type Target_Rep_Id is range 0 .. Integer'Last;
   ...
   --  The following procedure is a general destructor for nodes and entities

   procedure Destroy (NE : in out Node_Or_Entity_Id);

   --  The following function is a general purpose hash for nodes and entities

   function Hash (Key : Node_Or_Entity_Id) return Bucket_Range_Type;
   --  Obtain the hash value of entity Key

   --  The following list is a general purpose list for nodes and entities

   package NE_List is new Doubly_Linked_List
     (Element_Type    => Node_Or_Entity_Id,
      "="             => "=",
      Destroy_Element => Destroy);

   --  The following map is a general purpose map that relates nodes and
   --  entities to lists of nodes and entities.

   package NE_List_Map is new Dynamic_HTable
     (Key_Type              => Node_Or_Entity_Id,
      Value_Type            => NE_List.Instance,
   ...
   --  The following membership set is a general purpose set for nodes and
   --  entities.

   package NE_Set is new Membership_Set
     (Element_Type => Node_Or_Entity_Id,
      "="          => "=",
      Hash         => Hash);
   ...
   --  traversal performed by Traverse_Body.
   ...
   package Active_Scenarios is
      ...
      --  The following table stores all active scenarios in a recursive
      --  traversal starting from a top-level scenario. This table must be
      --  maintained in a FIFO fashion.
   ...
   package Activation_Processor is
      ...
      type Activation_Processor_Ptr is not null access procedure
        (Call     : Node_Id;
   ...
   --  bodies and processing eligible scenarios within.
   ...
   package Body_Processor is
      ...
      type Scenario_Predicate_Ptr is not null access function
        (N : Node_Id) return Boolean;
      --  Reference to a function which determines whether arbitrary node N
      --  denotes a suitable scenario for processing.

      type Scenario_Processor_Ptr is not null access procedure
   ...
   package Check_Installer is
      ...
      --  Install a conditional ABE check for scenario N with target Targ_Id.
      --  Targ_Rep is the representation of the target.
   ...
   package Conditional_ABE_Processor is
      ...
      --  Top level dispatcher for arbitrary scenarios. Perform conditional ABE
      --  checks and diagnostics for scenario N. In_State is the current state
   ...
   package Guaranteed_ABE_Processor is
      ...
      --  Top-level dispatcher for processing of scenarios which result in
      --  a guaranteed ABE. In_State is the current state of the Processing
      --  phase.
   ...
   package Internal_Representation is
      ...
      --  The following type enumerates all possible Ghost code mode kinds
      ...
      function Activated_Task_Objects
        (S_Id : Scenario_Rep_Id) return NE_List.Instance;
      pragma Inline (Activated_Task_Objects);
      --  For a Task_Activation_Scenario with id S_Id, obtain the list of task
      --  objects the scenario is activating.

      function Activated_Task_Type (S_Id : Scenario_Rep_Id) return Entity_Id;
      pragma Inline (Activated_Task_Type);
      --  For a Task_Activation_Scenario with id S_Id, obtain the currently
      --  activated task type.

      function Elaboration_Checks_OK (S_Id : Scenario_Rep_Id) return Boolean;
      pragma Inline (Elaboration_Checks_OK);
      --  Determine whether a scenario with id S_Id may be subjected to
      --  elaboration checks.
      ...
      --  For a Subprogram_Target with id T_Id, obtain the declaration of the
      --  barrier function's body.

      function Body_Declaration (T_Id : Target_Rep_Id) return Node_Id;
      pragma Inline (Body_Declaration);
      --  Obtain the declaration of the body which belongs to a target with id
      --  T_Id.

      function Elaboration_Checks_OK (T_Id : Target_Rep_Id) return Boolean;
      pragma Inline (Elaboration_Checks_OK);
      --  Determine whether a target with id T_Id may be subjected to
      --  elaboration checks.
      ...
      --  For a Variable_Target with id T_Id, obtain the declaration of the
      --  variable.
   ...
   --  invocation graph in the main unit's ALI file.

   package Invocation_Graph is

      Contains_Generics : Boolean := False;
      --  ???
   ...
   --  Output information concerning call Call that invokes subprogram Subp_Id.
   --  When flag Info_Msg is set, the routine emits an information message,
   --  otherwise it emits an error. When flag In_SPARK is set, " in SPARK" is
   ...
   --  a different unit. In_State is the current state of the Processing phase.
   ...
   package body Activation_Processor is
      ...
      is
         ...
         procedure Process_Task_Objects (Task_Objs : NE_List.Instance);
         ...
         procedure Process_Task_Object (Obj_Id : Entity_Id; Typ : Entity_Id) is
            Base_Typ : constant Entity_Id :=
                         Non_Private_View (Base_Type (Typ));

            Base_Rep : Target_Rep_Id;
            Comp_Id  : Entity_Id;
            Obj_Rep  : Target_Rep_Id;
         ...
         begin
            if Is_Task_Type (Typ) then
               Base_Rep := Target_Representation_Of (Base_Typ, New_In_State);
               ...
               New_In_State.Suppress_Warnings :=
                 New_In_State.Suppress_Warnings
                   or else not Elaboration_Warnings_OK (Base_Rep)
               ...
               Set_Activated_Task_Type (Call_Rep, Base_Typ);
         ...
         procedure Process_Task_Objects (Task_Objs : NE_List.Instance) is
   ...
   package body Body_Processor is
      ...
      Nested_Scenarios_Map : NE_List_Map.Instance := NE_List_Map.Nil;

      --  The following set contains all subprogram bodies that have been
      --  processed by routine Traverse_Body.

      Traversed_Bodies_Set : NE_Set.Instance := NE_Set.Nil;
      ...
      function Nested_Scenarios (N : Node_Id) return NE_List.Instance;
      ...
      function Nested_Scenarios (N : Node_Id) return NE_List.Instance is
      begin
         pragma Assert (Present (N));
      ...
      procedure Traverse_Body
        (N                   : Node_Id;
         ...
         In_State            : Processing_In_State)
      is
         Scenarios : NE_List.Instance := NE_List.Nil;
         ...
         begin
            ...
            elsif Requires_Processing (Scen) then
               Processor (Scen, In_State);
            end if;
   ...
   begin
      ...
      --  Examine all top-level scenarios saved during the Recording phase
      --  applying the Ada or SPARK elaboration rules in order to detect and
      --  diagnose ABE issues, install conditional ABE checks, and ensure the
      --  elaboration of units.
      ...
      --  Find all invocable constructs of the main unit which may participate
      --  in the elaboration of the main unit itself, or some other units which
      --  withs the main unit. Record the invocation relations in a path that
      --  reaches an external target.
   ...
   package body Check_Installer is
      ...
      procedure Install_ABE_Check_Or_Failure (N : Node_Id; Check : Node_Id);
      pragma Inline (Install_ABE_Check_Or_Failure);
      --  ???
      ...
      ----------------------------------
      -- Install_ABE_Check_Or_Failure --
      ----------------------------------

      procedure Install_ABE_Check_Or_Failure (N : Node_Id; Check : Node_Id) is
      ...
      begin
         ...
         Pop_Scope;
      end Install_ABE_Check_Or_Failure;
      ...
      --  Start for processing for Install_Scenario_ABE_Check
   ...
   begin
      --  An ABE check or failure is not needed when the compilation will not
      ...
      --  An ABE check or failure must not be installed when exceptions are not
      --  desirable.
   ...
   package body Conditional_ABE_Processor is
      ...
      begin
         while NE_Set.Has_Next (Iter) loop
            ...
            --  Reset the traversed status of all subprogram bodies to account
            --  for the new mode of scenario processing.
      ...
      begin
         ...
         elsif Is_Up_Level_Target (Spec_Decl, New_In_State) then
            return;
      ...
      begin
         ...
         elsif Is_Up_Level_Target (Subp_Decl, New_In_State) then
            return;
         end if;
      ...
      begin
         ...
         --  therefore Gen must not be considered if this is not the case.
                 ...
                 (Target_Decl => Spec_Declaration (Gen_Rep),
                  In_State    => New_In_State)
         then
            return;
         end if;
   ...
   package body Early_Call_Region_Processor is
      ...
      package ECR is new Dynamic_HTable
        (Key_Type              => Entity_Id,
         Value_Type            => Node_Id,
         No_Value              => Empty,
         Expansion_Threshold   => 1.5,
      ...
      Early_Call_Regions_Map : ECR.Instance := ECR.Nil;
      ...
      begin
         ...
         return ECR.Get (Early_Call_Regions_Map, Body_Id);
      ...
      begin
         ECR.Destroy (Early_Call_Regions_Map);
      ...
      begin
         Early_Call_Regions_Map := ECR.Create (100);
      ...
      begin
         ...
         ECR.Put (Early_Call_Regions_Map, Body_Id, Start);
   ...
   package body Elaborated_Units is
      ...
      --  Destroy elaboration attributes with id EA_Id

      package UA is new Dynamic_HTable
        (Key_Type              => Entity_Id,
      ...
      Unit_To_Attributes_Map : UA.Instance := UA.Create (250);
      ...
      --  Obtain the Elaborate[_All] pragma of elaboration attributes with id
      --  EA_Id.
      ...
      --  Determine whether elaboration attributes with id UA_Id exist

      procedure Set_Elab_Pragma
        (EA_Id : Elaboration_Attributes_Id;
         Prag  : Node_Id);
      pragma Inline (Set_Elab_Pragma);
      --  Set the Elaborate[_All] pragma of elaboration attributes with id
      --  EA_Id to Prag.
      ...
      --  Set the with clause of elaboration attributes with id EA_Id to Clause
      ...
      begin
         EA_Id := UA.Get (Unit_To_Attributes_Map, Unit_Id);
         ...
         if not Present (EA_Id) then
            ...
            UA.Put (Unit_To_Attributes_Map, Unit_Id, EA_Id);
         end if;
      ...
      begin
         UA.Destroy (Unit_To_Attributes_Map);
   ...
   package body Guaranteed_ABE_Processor is
      ...
      begin
         ...
         if Is_Up_Level_Target (Spec_Decl, In_State) then
            return;
   ...
   function Hash (Key : Node_Or_Entity_Id) return Bucket_Range_Type is
   begin
   ...
   package body Internal_Representation is
      ...
      type Scenario_Rep_Record is record
         ...
         List_1  : NE_List.Instance  := NE_List.Nil;
      end record;
      ...
      --  Destroy a target representation with id T_Id

      package ETT is new Dynamic_HTable
        (Key_Type              => Entity_Id,
      ...
      Entity_To_Target_Map : ETT.Instance := ETT.Create (500);

      --  The following map relates scenario representations to nodes

      procedure Destroy (S_Id : in out Scenario_Rep_Id);
      --  Destroy a scenario representation with id S_Id

      package NTS is new Dynamic_HTable
        (Key_Type              => Node_Id,
      ...
      Node_To_Scenario_Map : NTS.Instance := NTS.Create (500);
      ...
      pragma Inline (Present);
      --  Determine whether a scenario representation with id S_Id exists

      function Present (T_Id : Target_Rep_Id) return Boolean;
      pragma Inline (Present);
      --  Determine whether a target representation with id T_Id exists
      ...
      --  Obtain the version of a target representation with id T_Id
      ...
      begin
         ETT.Destroy (Entity_To_Target_Map);
         NTS.Destroy (Node_To_Scenario_Map);
      ...
      begin
         S_Id := NTS.Get (Node_To_Scenario_Map, N);
         ...
         if not Present (S_Id) then
            ...
            NTS.Put (Node_To_Scenario_Map, N, S_Id);
         end if;
      ...
      begin
         T_Id := ETT.Get (Entity_To_Target_Map, Id);
         ...
         if not Present (T_Id) then
            ...
            ETT.Put (Entity_To_Target_Map, Id, T_Id);
...
--    function Version (S_Id : Scenario_Rep_Id) return Representation_Kind is
--    begin
--       pragma Assert (Present (S_Id));
--       return Scenario_Reps.Table (S_Id).Version;
--    end Version;
   ...
   package body Invocation_Graph is
      ...
      type Simple_Invocation_Relation is record
      ...
      Saved_Constructs_Set : NE_Set.Instance := NE_Set.Nil;
      ...
      function Hash
        (Key : Simple_Invocation_Relation) return Bucket_Range_Type;
      --  Obtain the hash value of pair Key

      package IR_Set is new Membership_Set
        (Element_Type => Simple_Invocation_Relation,
         "="          => "=",
         Hash         => Hash);

      Saved_Relations_Set : IR_Set.Instance := IR_Set.Nil;
      ...
      function Is_Saved_Relation
        (Rel : Simple_Invocation_Relation) return Boolean;
      ...
      --  Record an invocation relation with predecessor Pred and successor
      --  Succ by creating an entry for it in the ALI file of the main unit.
      --  In_State denotes the current state of the Processing phase.
      ...
      procedure Set_Is_Saved_Relation
        (Rel : Simple_Invocation_Relation;
         Val : Boolean := True);
      ...
      is
         ...
         Invoker : constant Entity_Id := Key.Invoker;
         Target  : constant Entity_Id := Key.Target;
      ...
      begin
         ...
         --
         --    1) The low bits of the invoker are obtained, then shifted over
         --       to the high bits position.
         --
         --    2) The low bits of the target are obtained.
         --
         --  The results from 1) and 2) are or-ed to produce a value within the
         --  range of Bucket_Range_Type.

         return
           ((Bucket_Range_Type (Invoker) and Mask) * Half)  --  (1)
               or
            (Bucket_Range_Type (Target)  and Mask);         --  (2)
      end Hash;
      ...
      begin
         ...
         while Present (Decl) loop
            ...
            --  Stand-alone expression function, operator, or subprogram
      ...
      begin
         ...
         --  The task type being activated is within the main unit

         elsif In_Extended_Main_Code_Unit (Task_Typ) then
      ...
      begin
         ...
         --  The subprogram being called is within the main unit

         elsif In_Extended_Main_Code_Unit (Subp_Id) then
      ...
      begin
         ...
         --  Remove the current scenario from the stack of active scenarios
         --  once all invocation constructs are paths have been saved.
      ...
      begin
         ...
         --  package for invocation constructs.
            ...
            if Contains_Generics and then Present (Body_Id) then
               Process_Declarations
      ...
      begin
         ...
         --  Create a dummy scenario which calls the subprogram to act as a
         --  root for a DFS traversal.

         if not Is_Generic_Subprogram (Subp_Id) then
            Process_Invocation_Scenario
      ...
      begin
         --  Nothing to do when the task is not an invocation target

         if not Is_Invocation_Target (Task_Typ) then
            return;
         end if;
      ...
      procedure Record_Invocation_Path (In_State : Processing_In_State) is
         ...
         procedure Record_Full_Invocation_Path;
         ...
         function Target_Of (Pos : Active_Scenario_Pos) return Entity_Id;
         pragma Inline (Target_Of);
         --  Given position within the active scenario stack Pos, obtain the
         --  target of the indicated scenario.

         procedure Trace_Path (From : Entity_Id; To : Entity_Id);
         pragma Inline (Trace_Path);
         --  ???
         ...
         procedure Record_Full_Invocation_Path is
            First : Entity_Id;
            Last  : Entity_Id;

         begin
            --  The path originates from the elaboration of the body. Add an
            --  extra relation from the elaboration body procedure to the first
            --  active scenario.
            ...
            if In_State.Processing = Invocation_Body_Processing then
               ...
               Record_Invocation_Relation
                 (Pred     => Elab_Body_Id,
                  Succ     => Target_Of (Scenarios.First),
                  In_State => In_State);

            --  The path originates from the elaboration of the spec. Add an
            --  extra relation from the elaboration spec procedure to the first
            --  active scenario.
               ...
               First := Target_Of (Scenarios.First);
            end if;

            Last := Target_Of (Scenarios.Last);

            --  Record individual relations formed by pairs of scenarios

            for Index in Scenarios.First .. Scenarios.Last - 1 loop
               Record_Invocation_Relation
                 (Pred     => Target_Of (Index),
                  Succ     => Target_Of (Index + 1),
                  In_State => In_State);
            end loop;

            if First /= Last then
               Trace_Path (First, Last);
            end if;
         ...
         procedure Record_Simple_Invocation_Path is
            First : Entity_Id;
            Last  : Entity_Id;
         ...
         function Target_Of (Pos : Active_Scenario_Pos) return Entity_Id is
         ...
         procedure Trace_Path (From : Entity_Id; To : Entity_Id) is
         begin
            if not Debug_Flag_Underscore_TT then
               return;
            end if;

            Write_Name (Get_Qualified_Name (From));
            Write_Str  (" --> ");
            Write_Name (Get_Qualified_Name (To));
            Write_Eol;
         end Trace_Path;

      --  Start of processing for Record_Invocation_Path
      ...
      is
         ...
         --  relation with predecessor Pred and successor Succ.
         ...
         begin
            ...
            if Is_Accept_Alternative_Proc (Succ) then
               Extra := Receiving_Entry (Succ);
               Kind  := Accept_Alternative;

            --  Kirtchev ??? can Is_Activation_Proc execute now?

            --  Activation of a task object

            elsif Is_Activation_Proc (Succ) or else Is_Task_Type (Succ) then
               Extra := Empty;
               Kind  := Task_Activation;

            --  Controlled adjustment actions

            elsif Is_Controlled_Proc (Succ, Name_Adjust)
              or else Is_TSS (Succ, TSS_Deep_Adjust)
            then
               Extra := First_Formal_Type (Succ);
               Kind  := Controlled_Adjustment;

            --  Controlled finalization actions

            elsif Is_Controlled_Proc (Succ, Name_Finalize)
              or else Is_Finalizer_Proc (Succ)
              or else Is_TSS (Succ, TSS_Deep_Finalize)
            then
               Extra := First_Formal_Type (Succ);
            ...
            elsif Is_Controlled_Proc (Succ, Name_Initialize)
              or else Is_TSS (Succ, TSS_Deep_Initialize)
            then
               Extra := First_Formal_Type (Succ);
            ...
            elsif Is_Default_Initial_Condition_Proc (Succ) then
               Extra := First_Formal_Type (Succ);
               Kind  := Default_Initial_Condition_Verification;

            --  Initialization of object

            elsif Is_Init_Proc (Succ) then
               Extra := First_Formal_Type (Succ);
               Kind  := Type_Initialization;

            --  Initial_Condition verification

            elsif Is_Initial_Condition_Proc (Succ) then
               Extra := First_Formal_Type (Succ);
               Kind  := Initial_Condition_Verification;

            --  Instantiation

            elsif Is_Generic_Unit (Succ) then
               Extra := Empty;
               Kind  := Instantiation;

            --  Invariant verification

            elsif Is_Invariant_Proc (Succ)
              or else Is_Partial_Invariant_Proc (Succ)
            then
               Extra := First_Formal_Type (Succ);
            ...
            elsif Is_Postconditions_Proc (Succ) then
               Extra := Find_Enclosing_Scope (Succ);
               Kind  := Postcondition_Verification;

            --  Protected entry call

            elsif Is_Protected_Entry (Succ) then
               Extra := Empty;
               Kind  := Protected_Entry_Call;

            --  Protected subprogram call

            elsif Is_Protected_Subp (Succ) then
               Extra := Empty;
               Kind  := Protected_Subprogram_Call;

            --  Task entry call

            elsif Is_Task_Entry (Succ) then
               Extra := Empty;
      ...
      begin
         ...
         --  Declare the predecessor in the ALI file
         ...
         IR_Rec.Invoker := Signature_Of (Pred);
         IR_Rec.Kind    := Kind;
         IR_Rec.Target  := Signature_Of (Succ);
      ...
      function Signature_Of (Id : Entity_Id) return Invocation_Signature_Id is
         function Instantiation_Locations (Loc : Source_Ptr) return Name_Id;
         ...
         function Instantiation_Locations (Loc : Source_Ptr) return Name_Id is
         ...
         return
           Invocation_Signature_Of
             (Column    => Nat (Get_Column_Number (Loc)),
              ...
              Locations => Instantiation_Locations (Loc),
              Name      => Chars (Id),
              Scope     => Get_Qualified_Name (Scope (Id)));
      end Signature_Of;
   ...
   begin
      --  To qualify, the entity must denote a function or a procedure which is
      --  hidden within an anonymous package, and is a generic instance.
   ...
   begin
      ...
      --  root is always within the main unit.
   ...
   package body Scenario_Storage is
      ...
      Declaration_Scenarios       : NE_Set.Instance := NE_Set.Create (1000);
      Dynamic_ABE_Check_Scenarios : NE_Set.Instance := NE_Set.Create (500);
      Library_Body_Scenarios      : NE_Set.Instance := NE_Set.Create (1000);
      Library_Spec_Scenarios      : NE_Set.Instance := NE_Set.Create (1000);
      SPARK_Scenarios             : NE_Set.Instance := NE_Set.Create (100);
      ...
      procedure Replace_Scenario (Old_N : Node_Id; New_N : Node_Id) is
         procedure Replace_Scenario_In (Scenarios : NE_Set.Instance);
         ...
         procedure Replace_Scenario_In (Scenarios : NE_Set.Instance) is
         begin
